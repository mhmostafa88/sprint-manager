...
const StoriesForm = ({ isVisibleFoEdit }) => {
  ...
  const { addStory, clearStoriesList, storyToEdit, editStory } =
  ...
  return (
    <>
      {isStoryFormVisible || isVisibleFoEdit ? (
        <>
          <StoryForm>
            <button onClick={toggleVisibility}>Cancel Story Add</button>
